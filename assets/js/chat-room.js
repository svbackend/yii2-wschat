...
Chat.Room.prototype.addEventsHandlers = function () {
    ...
    Chat.vent.on('message:send', function (msg) {
        ...
        self.currentUser.set('message', msg.message);
        //self.currentUser.set('to', msg.to);
    ...
    try {
        ...
        switch (response.type) {
            case 'auth':
                ...
                break;
            case 'message':
                var user = new Chat.Models.User();

                console.log(response);

                user.set('type', 'info');
                user.set('message', response.data.message);
...
Chat.Room.prototype.sendMessage = function (data) {
    this.send({type: 'message', data: {message: data.get('message'), to: data.get('to')}});
};
